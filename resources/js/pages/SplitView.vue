<template>
    <div ref="containerEl" class="flex flex-col h-full overflow-hidden bg-bg-terminal">
        <!-- Header Multi-Stream -->
        <header
            class="shrink-0 bg-bg-terminal border-b border-primary/20 px-4 py-2 flex items-center justify-between"
        >
            <div class="flex items-center gap-3">
                <RouterLink
                    :to="{ name: 'projects' }"
                    class="text-primary/40 hover:text-primary transition-all"
                >
                ...
                <h2 class="text-xs font-display text-primary glow uppercase tracking-[0.3em]">
                    Multi_Stream_Matrix
                </h2>
                <span class="text-[9px] text-primary/30 font-mono hidden sm:inline"
                    >Active_Nodes: {{ sessionIds.length }}</span
                >
            </div>
        ...
        <div
            class="md:hidden flex border-b border-primary/20 shrink-0 overflow-x-auto custom-scrollbar"
        >
            <button
                ...
                :class="
                    activeMobilePanel === index
                        ? 'text-primary bg-primary/10 border-b-2 border-primary glow'
                        : 'text-primary/40'
                "
        ...
        >
            <div
                ...
                :class="[isDesktop ? (layout === 'cols' ? 'flex-1 border-r' : 'border') : 'flex-1']"
            >
                <!-- Stream Toolbar -->
                <div
                    class="bg-primary/5 px-4 py-1 flex items-center justify-between border-b border-primary/5"
                >
                ...
                >
                    ...
                    <div class="flex-1 overflow-y-auto py-2 custom-scrollbar">
                        ...
                        >
                            <div class="min-w-0">
                                <p
                                    class="text-xs font-bold text-primary/80 group-hover:text-primary uppercase truncate"
                                >
                                    {{ s.title || s.initial_instruction || 'UNTITLED_STREAM' }}
                                </p>
                                <p class="text-[9px] text-primary/30 uppercase mt-1">
                                    {{ s.mode }} | {{ s.status }}
                                </p>
                            </div>
                            <span class="text-[9px] font-mono text-primary/20"
                                >[{{ s.id.substring(0, 8) }}]</span
                            >
                        </button>
                    </div>
...
function removeSession(id) {
    const newIds = sessionIds.value.filter((sid) => sid !== id);
    if (newIds.length === 0) {
        router.push({ name: 'projects' });
    } else if (newIds.length === 1) {
        router.push({ name: 'session', params: { id: newIds[0] } });
    } else {
        if (activeMobilePanel.value >= newIds.length) {
            activeMobilePanel.value = newIds.length - 1;
        }
        router.push({ name: 'multi', params: { ids: newIds.join(',') } });
    }
}
