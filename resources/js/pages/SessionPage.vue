      ...
      >
        <div class="bg-black border border-primary border-double w-full max-w-lg flex flex-col max-h-[80vh] relative">
          ...
          <div class="flex items-center justify-between px-6 py-5 border-b border-primary/20 shrink-0">
            <div>
              <h3 class="text-xl font-display text-primary glow uppercase tracking-widest">Parallel_Link</h3>
              <p class="text-[10px] text-primary/40 uppercase tracking-widest mt-1">Select node for concurrent stream</p>
            </div>
            <button @click="closePicker" class="text-primary/40 hover:text-primary transition-colors">
          ...
          <div class="px-6 py-4 border-b border-primary/10 shrink-0">
            <label class="block text-[9px] font-bold text-primary/60 uppercase tracking-widest mb-2">Target_Node</label>
            <select
          ...
          <div class="flex-1 overflow-y-auto py-2 custom-scrollbar">
            <div v-if="pickerLoading" class="px-6 py-8 text-[10px] text-primary/40 uppercase animate-pulse">
              Accessing_Data...
            </div>
            <div v-else-if="pickerSessions.length === 0" class="px-6 py-8 text-[10px] text-primary/40 uppercase border border-dashed border-primary/10 mx-6 my-4 text-center">
            ...
            >
              <div class="min-w-0">
                <p class="text-xs font-bold text-primary/80 group-hover:text-primary group-hover:glow uppercase tracking-wider truncate">
                ...
                <div class="flex items-center gap-3 mt-2">
                  ...
                  >
                    <span v-if="isActiveStatus(s.status)" class="w-1 h-1 bg-current animate-pulse"></span>
                    {{ statusLabel(s.status) }}
                  </span>
                </div>
              </div>
              <span class="text-[9px] font-mono text-primary/30 shrink-0 mt-1 uppercase">
