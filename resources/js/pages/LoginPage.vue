<template>
  <div class="min-h-screen bg-black flex items-center justify-center p-4 selection:bg-primary selection:text-black">
    <div class="w-full max-w-md border border-primary-dim bg-black p-8 relative overflow-hidden group">
      <!-- Decorative corners -->
      ...
      <div class="flex flex-col items-center mb-10 text-center">
        <div class="w-16 h-16 border-2 border-primary flex items-center justify-center mb-4 glow-box">
           <span class="font-display text-4xl leading-none">>_</span>
        </div>
        <div>
          <h1 class="text-4xl font-display uppercase tracking-[0.2em] text-primary glow mb-1">gocode_OS</h1>
          <p class="text-[10px] text-primary/50 uppercase tracking-[0.3em]">Protocol: Authentication_Required</p>
        </div>
      </div>

      <form class="space-y-6" @submit.prevent="submit">
        <div class="space-y-1">
          <label class="block text-[10px] font-bold text-primary/60 uppercase tracking-widest ml-1">User_Identity</label>
          <div class="relative">
            <span class="absolute left-3 top-2.5 text-primary text-sm opacity-50">></span>
            <input
              v-model="form.email"
              type="email"
              required
        ...
        <div class="space-y-1">
          <label class="block text-[10px] font-bold text-primary/60 uppercase tracking-widest ml-1">Access_Key</label>
          <div class="relative">
            <span class="absolute left-3 top-2.5 text-primary text-sm opacity-50">*</span>
            <input
        ...
        <div v-if="error" class="p-2 border border-danger/50 bg-danger/10 text-danger text-[10px] uppercase font-bold text-center animate-pulse">
          [!] ACCESS_DENIED: {{ error }}
        </div>
      ...
      <div class="mt-8 pt-6 border-t border-primary/10 flex justify-between items-center text-[9px] text-primary/30 font-mono uppercase">
