  ...
  <div class="p-6 lg:p-8 selection:bg-primary selection:text-black">
    ...
    <div class="flex items-center justify-between mb-10 border-b border-primary-dim pb-6">
      <div class="flex items-center gap-4 min-w-0">
        ...
        <div class="min-w-0">
          <h2 class="text-3xl font-display uppercase tracking-widest text-primary glow truncate">Session_Log</h2>
          <div v-if="projectName" class="flex items-center gap-2 text-[10px] text-primary/40 uppercase tracking-widest mt-1">
            <span class="truncate">Node: {{ projectName }}</span>
    ...
    <!-- Liste des sessions -->
    <div v-if="store.loading" class="text-primary/50 text-[10px] uppercase animate-pulse">Accessing_Data_Streams...</div>
    <div v-else-if="store.sessions.length === 0" class="text-center py-20 border border-dashed border-primary/20">
      <p class="text-primary/40 text-[10px] uppercase tracking-[0.3em]">No active session streams detected in this node.</p>
    </div>
    <div v-else class="space-y-4">
      <RouterLink
        ...
        class="block bg-black border border-primary/20 p-5 hover:border-primary/60 hover:bg-primary/5 transition-all group relative overflow-hidden"
      >
        <!-- Scanner line effect on hover -->
        <div class="absolute inset-0 bg-gradient-to-b from-transparent via-primary/5 to-transparent -translate-y-full group-hover:translate-y-full transition-transform duration-1000"></div>

        <div class="flex items-start justify-between gap-4 relative z-10">
          <div class="flex-1 min-w-0">
            <div class="mb-3">
              <span class="text-sm font-bold text-primary group-hover:glow transition-all uppercase tracking-wider">
                {{ session.title || session.initial_instruction || 'UNTITLED_STREAM' }}
              </span>
            </div>
            <div class="flex items-center gap-4 flex-wrap">
              ...
              <!-- Cost -->
              <span v-if="session.cost_usd > 0" class="text-[9px] text-primary/40 font-mono uppercase">
                RESOURCE_DRAIN: ${{ formatCost(session.cost_usd) }}
              </span>
            </div>
          </div>
          <div class="text-[9px] text-primary/30 font-mono uppercase shrink-0 mt-1">
    ...
    <Teleport to="body">
      ...
      >
        <div class="bg-black border border-primary border-double w-full max-w-lg p-8 relative">
          <h3 class="text-2xl font-display text-primary glow uppercase tracking-widest mb-6">Initialize_Stream</h3>
          <form @submit.prevent="submitCreate" class="space-y-6">

            <!-- Config projet manquante -->
            <div v-if="!project?.path" class="border border-warning/50 bg-warning/5 p-4 space-y-4">
              <p class="text-[10px] text-warning font-bold uppercase tracking-widest">[!] PATH_NOT_DEFINED</p>
              <div class="space-y-1">
                <label class="block text-[9px] text-primary/60 uppercase font-bold">Base_Directory</label>
                <input
              ...
              <div class="space-y-1">
                <label class="block text-[9px] text-primary/60 uppercase font-bold">Registry_Name</label>
                <input
              ...
              <p v-if="setupFullPath" class="text-[9px] text-primary/30 font-mono">TARGET_MOUNT: {{ setupFullPath }}</p>
              <label class="flex items-center gap-2 cursor-pointer">
                <input
                  v-model="setup.git_init"
                  type="checkbox"
                  class="w-4 h-4 bg-black border-primary text-primary focus:ring-0"
                />
                <span class="text-[10px] text-primary/60 uppercase font-bold">Initialize_Git_Subsystem</span>
              </label>
            </div>

            <!-- Titre optionnel -->
            <div class="space-y-1">
              <label class="block text-[10px] font-bold text-primary/60 uppercase tracking-widest">Nom</label>
              <input
                v-model="form.title"
                type="text"
                ...
                placeholder="FIX_AUTH_CORE"
              />
            </div>

            <!-- Mode -->
            <div class="space-y-2">
              <label class="block text-[10px] font-bold text-primary/60 uppercase tracking-widest">Execution_Protocol</label>
              <div class="grid grid-cols-3 gap-3">
                <button
                  ...
                  class="flex flex-col items-center gap-1 px-2 py-3 border text-[10px] uppercase font-bold transition-all"
                ...
                >
                  <span>{{ m.label }}</span>
                  <span class="text-[8px] opacity-50 tracking-tighter">{{ m.desc }}</span>
                </button>
              </div>
            </div>
            ...
            <div class="space-y-1">
              <label class="block text-[10px] font-bold text-primary/60 uppercase tracking-widest">Primary_Directive</label>
              <textarea
                ...
                placeholder="DEFINE_TASK_PARAMETERS..."
              ></textarea>
            </div>
            ...
            <div class="flex justify-end gap-6 pt-2">
              ...
              >
                <span v-if="creating">Spawning...</span>
                <span v-else>[ EXECUTE_SPAWN ]</span>
              </button>
            </div>
          </form>
        </div>
