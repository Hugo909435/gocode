<template>
    <div
        class="flex flex-col h-full overflow-hidden bg-bg-terminal selection:bg-primary selection:text-black"
        :style="themeStyle"
    >
        <!-- ─── Header ─────────────────────────────────────────────── -->
        <header class="shrink-0 bg-bg-terminal border-b border-primary-dim px-4 py-3">
            <div class="flex items-center justify-between gap-3">
                ...
                <div class="flex items-center gap-4 min-w-0">
                    <RouterLink
                        ...
                        :to="
                            session
                                ? { name: 'project-sessions', params: { id: session.project_id } }
                                : { name: 'projects' }
                        "
                    ...
                    <div class="flex flex-col min-w-0">
                        <h1
                            class="text-sm font-display theme-text text-primary glow truncate leading-none mb-1"
                        >
                            {{
                                session?.title ||
                                session?.initial_instruction ||
                                (settings.theme === 'hacker' ? 'SESSION_STREAM' : 'Conversation')
                            }}
                        </h1>
                        <div class="flex items-center gap-2 text-[9px] text-primary/40 theme-text">
                ...
                <div class="flex items-center gap-3 shrink-0">
                    ...
                    <span
                        v-if="totalCost > 0"
                        class="text-[9px] text-primary/30 font-mono hidden sm:inline theme-text"
                    >
                        {{ settings.theme === 'hacker' ? 'DRAIN' : 'Cost' }}: ${{
                            totalCost.toFixed(4)
                        }}
                    </span>
                    <!-- Mode -->
                    ...
                    <span
                        v-if="session"
                        class="text-[9px] px-2 py-0.5 border font-bold theme-text flex items-center gap-1 rounded-[--radius-none]"
                        :class="statusBadgeClass(session.status)"
                    >
                        <span
                            v-if="isActive"
                            class="w-1 h-1 bg-current shadow-[0_0_5px_currentColor] animate-pulse rounded-full"
                        ></span>
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <button
                            ...
                            :disabled="clearing"
                            class="flex items-center gap-1.5 px-2 py-1 text-[9px] font-bold text-primary/50 hover:text-black bg-primary/5 hover:bg-danger hover:border-danger border border-primary/30 transition-all disabled:opacity-20 theme-text rounded-[--radius-none]"
                            :title="
                                settings.theme === 'hacker' ? 'PURGE_LOG' : 'Clear conversation'
                            "
                            @click="doClear"
                        >
                            {{ settings.theme === 'hacker' ? '[ CLR ]' : 'Clear' }}
                        </button>

                        <!-- Split View -->
                        <button
                            v-if="showSplitAction"
                            class="flex items-center gap-1.5 px-2 py-1 text-[9px] font-bold text-primary/70 hover:text-black bg-primary/5 hover:bg-primary border border-primary/30 transition-all theme-text rounded-[--radius-none]"
                            @click="emit('split')"
                        >
                            {{ settings.theme === 'hacker' ? '[ SPLIT ]' : 'Split' }}
                        </button>
                        ...
                        <button
                            ...
                            :disabled="isActive"
                            class="flex items-center gap-1.5 px-2 py-1 text-[9px] font-bold text-primary hover:text-black bg-primary/5 hover:bg-primary border border-primary/30 transition-all disabled:opacity-20 theme-text rounded-[--radius-none]"
                            @click="doLaunch"
                        >
                            {{ settings.theme === 'hacker' ? '[ RUN ]' : 'Execute' }}
                        </button>
                        ...
                        >
                            <input
                                ...
                                class="bg-bg-terminal border border-primary/20 px-2 py-1 text-[9px] font-mono text-primary placeholder:text-primary/20 focus:outline-none focus:border-primary/50 transition-colors w-32 rounded-[--radius-none]"
                            />
                            <button
                                :disabled="isActive || pushing"
                                class="flex items-center gap-1.5 px-2 py-1 text-[9px] font-bold text-primary/70 hover:text-black bg-primary/5 hover:bg-primary border border-primary/30 transition-all disabled:opacity-20 theme-text rounded-[--radius-none]"
                                @click="doPush"
                            >
                                {{
                                    pushing
                        ...
                        <div
                            ...
                            :class="
                                pollConnected
                                    ...
                                    : 'bg-danger shadow-[0_0_5px_var(--color-danger)]'
                            "
        ...
        <Transition name="launch-fade">
            <div
                v-if="launching && !launchFinished"
                class="shrink-0 bg-bg-terminal border-b border-primary-dim"
            >
                ...
                <div class="px-4 py-1.5 flex items-center justify-between">
                    <span class="text-[9px] text-primary/60 font-mono theme-text">{{
                        launchStatusText
                    }}</span>
                </div>
            </div>
        ...
        <Transition name="launch-fade">
            <div
                v-if="launchFinished && launchUrl"
                class="shrink-0 px-4 py-2 bg-primary/5 border-b border-primary/20 flex items-center justify-between gap-3 rounded-b-[--radius-none]"
            >
                <a
                    :href="launchUrl"
                    target="_blank"
                    rel="noopener"
                    class="inline-flex items-center gap-1.5 text-[10px] text-primary hover:glow transition-all theme-text font-bold"
                >
                    <svg
                    ...
                    {{ settings.theme === 'hacker' ? 'ACCESS_POINT' : 'Live Preview' }}:
                    {{ launchUrl }}
                </a>
                <button
                    class="text-primary/40 hover:text-primary transition-colors"
                    @click="closeLaunchOverlay"
                >
        ...
        >
            ...
            <div
                v-if="!loadingSession && messages.length === 0"
                class="flex flex-col items-center py-20 opacity-20 select-none"
            >
                <span class="font-display text-6xl mb-4">>_</span>
                <p class="text-[10px] theme-text">
                    {{ settings.theme === 'hacker' ? 'Waiting_for_directives' : 'Ready to help' }}
                </p>
            </div>
            ...
            <template v-for="msg in messages" :key="msg.id || msg._tempId">
                ...
                <div
                    v-if="
                        displayType(msg) === 'status' ||
                        displayType(msg) === 'done' ||
                        displayType(msg) === 'log' ||
                        displayType(msg) === 'cost'
                    "
                    class="flex items-center gap-3 py-0.5 opacity-40 hover:opacity-100 transition-opacity"
                >
                    <span class="text-[9px] font-mono text-primary/50 shrink-0"
                        >[{{
                            formatDate(msg.created_at || new Date().toISOString()).split(' ')[1]
                        }}]</span
                    >
                    <div class="w-1 h-1 bg-primary/60 shrink-0 rounded-full"></div>
                    <span class="text-[10px] font-mono theme-text">
                        <template v-if="displayType(msg) === 'cost'">
                            RESOURCE_USAGE: ${{ msg.content?.cost_usd?.toFixed(4) }} (IN:{{
                                msg.content?.input_tokens
                            }}
                            OUT:{{ msg.content?.output_tokens }})
                        </template>
                        <template v-else>
                            {{
                                msg.content?.message ||
                ...
                >
                    <div
                        class="flex items-center gap-2 text-[9px] font-bold text-primary/40 theme-text ml-1"
                    >
                        <span class="glow">{{
                            settings.theme === 'hacker' ? 'CORE_PROCESSOR' : 'Assistant'
                        }}</span>
                    </div>
                    <div
                        class="bg-primary/5 border-l-2 border-primary/60 px-4 py-3 text-[13px] text-primary/90 whitespace-pre-wrap leading-relaxed font-mono selection:bg-primary selection:text-black rounded-r-[--radius-none]"
                    >
                ...
                <div
                    ...
                    class="bg-bg-terminal border border-primary/40 overflow-hidden my-4 rounded-[--radius-none]"
                >
                    <div
                        class="flex items-center justify-between px-4 py-2 border-b border-primary/20 bg-primary/10"
                    >
                        <span class="text-[10px] font-bold text-primary theme-text">{{
                            settings.theme === 'hacker' ? 'Strategy_Map' : 'Implementation Plan'
                        }}</span>
                    </div>
                    <pre
                        class="px-4 py-4 text-xs text-primary/80 whitespace-pre-wrap font-mono leading-relaxed overflow-x-auto"
                        >{{ msg.content?.content || msg.content }}</pre
                    >
                </div>
                ...
                >
                    <div
                        class="w-2 h-2 border border-primary/60 rotate-45 shrink-0"
                        :class="settings.theme === 'modern' ? 'rounded-full' : ''"
                    ></div>
                    <div class="text-[10px] font-mono text-primary/60 theme-text">
                        <span class="text-primary/90">CALL::{{ msg.content?.tool }}</span>
                        <span v-if="msg.content?.file" class="text-primary/40">
                            -> {{ msg.content.file }}</span
                        >
                        <span v-else-if="msg.content?.command" class="text-primary/40">
                            -> {{ msg.content.command }}</span
                        >
                        <span v-else-if="msg.content?.params?.file_path" class="text-primary/40">
                            -> {{ msg.content.params.file_path }}</span
                        >
                    </div>
                </div>
                ...
                <div
                    ...
                    class="bg-bg-terminal border border-primary/20 overflow-hidden my-2 ml-4 rounded-[--radius-none]"
                >
                    <div
                        class="flex items-center gap-2 px-3 py-1.5 bg-primary/5 border-b border-primary/10"
                    >
                        <span class="text-[9px] font-mono text-primary/50"
                            >OUT::{{ msg.content?.command }}</span
                        >
                    </div>
                    <pre
                ...
                >
                    <div
                        class="flex items-center justify-between gap-2 px-3 py-1.5 bg-primary/5 border-b border-primary/10"
                    >
                        <span class="text-[9px] font-mono text-primary/70 uppercase"
                            >DIFF::{{ msg.content?.file }}</span
                        >
                    ...
                    <div class="overflow-x-auto max-h-96 overflow-y-auto">
                        <table class="w-full text-[11px] font-mono">
                            <tbody>
                                ...
                                >
                                    <td
                                        class="pl-2 pr-2 py-0 select-none opacity-30 text-right w-6 border-r border-primary/10"
                                    >
                                        {{ line.prefix }}
                                    </td>
                ...
                <div
                    ...
                    class="border-2 border-warning bg-warning/5 my-6 p-6 relative overflow-hidden rounded-[--radius-none]"
                >
                    ...
                    <div class="relative z-10">
                        <h4 class="text-warning font-display text-xl theme-text mb-2 glow-warning">
                            {{
                                settings.theme === 'hacker'
                        ...
                        <p class="text-sm text-warning/80 mb-6 font-mono tracking-tight">
                            {{ msg.content?.message }}
                        </p>
                        ...
                        <div
                            v-if="
                                ...
                                !confirmedActions.has(msg.content?.action_id)
                            "
                            class="flex gap-4"
                        >
                            <button
                                :disabled="confirming"
                                class="flex-1 py-3 bg-warning/10 border border-warning text-warning text-xs font-bold theme-text hover:bg-warning hover:text-black transition-all rounded-[--radius-none]"
                            ...
                            <button
                                :disabled="confirming"
                                class="flex-1 py-3 bg-bg-terminal border border-warning/40 text-warning/60 text-xs font-bold theme-text hover:border-warning hover:text-warning transition-all rounded-[--radius-none]"
                                @click="doConfirm(msg.content?.action_id, false)"
                            >
                                {{ settings.theme === 'hacker' ? '[ DENY ]' : 'Reject' }}
                            </button>
                        </div>
                        <div
                            v-else
                            class="text-[10px] font-bold theme-text text-warning/60 border-t border-warning/20 pt-4"
                        >
                            {{
                                settings.theme === 'hacker'
                                    ? 'PROTOCOL_RESOLVED'
                            ...
                            {{
                                confirmedActions.get(msg.content?.action_id)
                                    ? 'ACCESS_GRANTED'
                                    : 'ACCESS_DENIED'
                            }}
                        </div>
                ...
                <div
                    ...
                    class="bg-danger/5 border border-danger/40 p-4 my-2 flex gap-4 rounded-[--radius-none]"
                >
                    <div class="text-danger font-display text-2xl shrink-0 mt-0.5">!</div>
                    <div class="flex-1">
                        <h5 class="text-danger text-[10px] font-bold theme-text mb-1">
                            {{
                                settings.theme === 'hacker'
                        ...
                        <p class="text-xs text-danger/80 font-mono whitespace-pre-wrap">
                            {{ msg.content?.message || 'NULL_RESPONSE_EXCEPT' }}
                        </p>
                    </div>
                </div>
                ...
                >
                    ...
                    <div
                        class="max-w-[85%] bg-primary/10 border-r-2 border-primary px-4 py-2 text-[13px] text-primary whitespace-pre-wrap font-mono leading-relaxed rounded-l-[--radius-none]"
                    >
                        <span v-if="settings.theme === 'hacker'" class="text-primary/40 mr-2"
                            >></span
                        >{{ msg.content?.text || msg.content }}
                    </div>
                    <!-- Indicateur de chargement sous le dernier message utilisateur -->
                    <div
                        v-if="
                            (sending || isActive) &&
                            ...
                            msg === lastUserMessage
                        "
                        class="flex items-center gap-1.5 px-2 py-1 text-[9px] text-primary/50 font-mono"
                    >
                        <span
                            class="w-1 h-1 bg-primary/60 rounded-full animate-bounce"
            ...
            <div
                v-if="isActive && !isAwaitingConfirmation"
                class="flex items-center gap-3 py-2 ml-1"
            >
                <div class="w-1.5 h-1.5 bg-primary animate-ping rounded-full"></div>
                <span class="text-[10px] text-primary font-mono theme-text"
        ...
        <!-- ─── Barre d'instruction ────────────────────────────────── -->
        <div class="shrink-0 bg-bg-terminal border-t border-primary-dim p-6">
            <div class="flex flex-col gap-4 max-w-5xl mx-auto">
                <!-- Input Area -->
                <div class="relative group">
                    <div
                        v-if="settings.theme === 'hacker'"
                        class="absolute -inset-0.5 bg-primary/20 opacity-0 group-focus-within:opacity-100 transition-opacity blur-sm"
                    ></div>
                    <div
                        class="relative flex gap-4 items-end bg-bg-terminal border border-primary/30 p-2 group-focus-within:border-primary transition-colors rounded-[--radius-none] shadow-sm"
                    >
                        <div
                            v-if="settings.theme === 'hacker'"
                            class="pl-3 pb-3 shrink-0 text-primary/60 font-display text-xl leading-none select-none"
                        >
                            >_
                        </div>
                        ...
                        <textarea
                            ...
                            "
                            class="flex-1 bg-transparent text-primary text-[14px] px-3 py-2.5 focus:outline-none resize-none disabled:opacity-20 font-mono placeholder:text-primary/10 h-auto"
                        ...
                        <div class="flex gap-2 shrink-0 pb-1 pr-1">
                            <!-- Clear -->
                            <button
                                v-if="instruction.trim() && !isActive"
                                class="w-10 h-10 flex items-center justify-center bg-primary/5 border border-primary/20 text-primary/40 hover:text-primary hover:border-primary/60 transition-all rounded-[--radius-none]"
                                title="CLEAR"
                                @click="instruction = ''"
                            >
                                <svg
                            ...
                            <button
                                v-if="isActive"
                                :disabled="stopping"
                                class="w-10 h-10 flex items-center justify-center bg-danger/10 border border-danger/40 text-danger hover:bg-danger hover:text-black transition-all disabled:opacity-20 rounded-[--radius-none]"
                                title="ABORT"
                                @click="doStop"
                            >
                            ...
                            <button
                                ...
                                class="w-10 h-10 flex items-center justify-center bg-primary/10 border border-primary/40 text-primary hover:bg-primary hover:text-black transition-all disabled:opacity-20 rounded-[--radius-none]"
                                title="EXECUTE"
                ...
                <div class="flex justify-between items-center px-1">
                    <div class="flex gap-4">
                        <button
                            v-for="m in modes"
                            :key="m.value"
                            class="text-[10px] font-bold theme-text transition-all"
                            :class="
                    ...
                    <span class="text-[9px] text-primary/20 theme-text hidden sm:inline">{{
                        settings.theme === 'hacker'
                            ? 'Ctrl+Enter_to_Execute'
...
const props = defineProps({
    ...
    emitBack: { type: Boolean, default: false },
...
const themeStyle = computed(() => {
    const colors = {
        primary: settings.theme === 'modern' ? '#6366f1' : '#00ff41',
        blue: '#00d2ff',
        orange: '#ff9d00',
        purple: '#d000ff',
    };
...
const isActive = computed(() => session.value && activeStatuses.has(session.value.status));

const isAwaitingConfirmation = computed(() => session.value?.status === 'awaiting_confirmation');

const totalCost = computed(() => parseFloat(session.value?.cost_usd ?? 0));

const lastUserMessage = computed(() => {
    const userMsgs = messages.value.filter((m) => displayType(m) === 'user');
    return userMsgs.at(-1) ?? null;
});
...
async function doClear() {
    if (clearing.value || isActive.value) return;
    clearing.value = true;
    try {
        const updated = await store.clearMessages(sessionId.value);
        session.value = updated;
        messages.value = [];
        pollCursor = 0;
        confirmedActions.value = new Map();
    } catch (e) {
...
async function saveRemote() {
    ...
    try {
        ...
        session.value = {
            ...session.value,
            project: { ...session.value.project, git_remote: data.data.git_remote },
        };
        closeRemoteForm();
    } catch (e) {
...
async function createGitHubRepo() {
    ...
    try {
        const { data } = await api.post(
            `/projects/${session.value.project_id}/github/create-repo`,
            {
